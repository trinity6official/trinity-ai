...
class DebugSkill:
    ...
    def get_llm_status(self):
        """Check Trinity's local-only AI runtime and configured task models."""
        import os
        import requests as _requests

        local_url = os.environ.get('LOCAL_LLM_URL', 'http://127.0.0.1:11434')
        configured = {
            'fast': os.environ.get('TRINITY_FAST_MODEL', 'qwen3:8b'),
            'general': os.environ.get('TRINITY_GENERAL_MODEL', 'qwen3:14b'),
            'reasoning': os.environ.get('TRINITY_REASONING_MODEL', 'qwen3:30b'),
            'coding': os.environ.get('TRINITY_CODING_MODEL', 'qwen3:14b'),
        }
        vision_model = os.environ.get('TRINITY_VISION_MODEL', '').strip()

        try:
            resp = _requests.get(f"{local_url}/api/tags", timeout=2)
            resp.raise_for_status()
            installed = sorted(m.get('name', '') for m in resp.json().get('models', []) if m.get('name'))
            installed_set = set(installed)
            task_status = {
                task: {'model': model, 'installed': model in installed_set}
            ...
            return {
                ...
                'url': local_url,
                'available': True,
                'task_models': task_status,
                'vision_model': {
                    'model': vision_model or None,
                    'installed': bool(vision_model and vision_model in installed_set),
                },
                'installed_models': installed,
                'current_default': configured['general'],
                'recommendation': 'Ready' if all(v['installed'] for v in task_status.values()) else 'Install the missing configured local models',
            }
            ...
            return {
                ...
                'url': local_url,
                'available': False,
                'task_models': {task: {'model': model, 'installed': False} for task, model in configured.items()},
                'vision_model': {'model': vision_model or None, 'installed': False},
                'installed_models': [],
                'current_default': configured['general'],
                'recommendation': f'Start the local model runtime: {exc}',
            }

...
def test_invalid_new_skill_name_is_rejected(tmp_path):
    ...
    result = SkillEvolutionService(host, tmp_path).propose_new_skill("bad/name", "x")
    assert result[0] is False


def test_missing_existing_skill_is_not_created_by_tool_extension(tmp_path):
    host = SimpleNamespace()
    assert SkillEvolutionService(host, tmp_path).propose_missing_tool("missing", "read_x", {}, object()) is False
...
def test_missing_tool_is_drafted_but_not_written_until_approval(tmp_path):
    ...
    skills = MagicMock(); skills.execute_request.return_value = {"success": True}; skills.get_skill.return_value = object()
    ...
    assert service.propose_missing_tool("demo", "read_x", {}, object()) is False
    assert skill.read_text() == original
    proposal_id = next(iter(service.get_pending_changes()))
    assert skills.execute_request.call_count == 0
...
def test_new_skill_is_only_created_after_approval(tmp_path):
    ...
    host._invoke_with_failover = lambda *a, **k: SimpleNamespace(content=generated)
    service = SkillEvolutionService(host, tmp_path)
    ok, message = service.propose_new_skill("email", "Inbox triage")
...
def test_legacy_evolution_method_names_delegate_to_governed_proposals(tmp_path):
    service = SkillEvolutionService(SimpleNamespace(), tmp_path)
    service.propose_missing_tool = MagicMock(return_value=False)
    service.propose_new_skill = MagicMock(return_value=(False, "approval required"))

...
LLM. It searches the local knowledge index only when wording suggests personal
files/notes are likely to contain relevant evidence.
"""
...
class KnowledgeRetrievalPolicy:
    ...
    _EXPLICIT_PHRASES = (
        ...
        "in the pdf", "in my pdf", "in our pdf", "in the docx",
    )
    ...
    _PATH_RE = re.compile(
        r"(?:[/\\~][^\s]+)|(?:\b[\w.-]+\.(?:md|txt|rst|json|ya?ml|toml|csv|py|sh|go|rb|js|ts|sql|pdf|docx)\b)",
        re.IGNORECASE,
    )
...
class KnowledgeRetrievalService:
    ...
    def retrieve(self, question: str, skill_manager: Any) -> KnowledgeRetrievalContext:
        ...
        if permission_engine is not None:
            permission = permission_engine.assess_tool("knowledge", "search_knowledge")
        ...
        result = skill_manager.execute(
            "knowledge",
            "search_knowledge",
            {"query": str(question), "limit": self.max_results, "mode": "hybrid"},
        )
        ...
        return KnowledgeRetrievalContext(True, decision.reason, context=context, source_refs=refs)

    def _format_results(self, rows: list[Any]) -> tuple[str, tuple[str, ...]]:
        sections: list[str] = []
        refs: list[str] = []
        used_chars = 0

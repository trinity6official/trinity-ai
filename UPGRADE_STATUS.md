...
- Cached MCP tool discoveries are indexed by the Capability Registry as runtime-only metadata with stable `mcp:<server>.<tool>` IDs; registry reads never launch MCP processes or perform discovery I/O, and execution remains disabled until governance.
